...
Sau khi mua vé thành công, khán giả nhận thông báo xác nhận qua app và email kèm e-ticket. Khi concert sắp diễn ra (trước 24 giờ), hệ thống gửi nhắc nhở tự động.
...
Các địa điểm tổ chức concert lớn, như sân vận động hoặc nhà thi đấu, thường có vùng sóng không ổn định khi hàng chục nghìn người tập trung. Vì vậy, app phải cho phép ghi nhận soát vé tạm thời khi không có mạng và tự đồng bộ lại khi kết nối được phục hồi.
...
Một số concert có khu vực Guest List dành cho khách mời của nhãn hàng tài trợ. Hệ thống quản lý khách mời của nhãn hàng không có API — cách duy nhất là nhận file CSV mà nhãn hàng gửi vào ban đêm trước ngày diễn.
...
Trang danh sách concert và trang chi tiết từng concert bị đọc với tần suất rất cao, có thể hàng nghìn lần/giây trong giờ cao điểm, nhưng dữ liệu thay đổi không thường xuyên.
...
Phân rã hệ thống thành các container, ví dụ:
...
Chỉ rõ công nghệ đề xuất và cách các container giao tiếp với nhau.

### 3. High-Level Architecture Diagram

Vẽ sơ đồ kiến trúc tổng quan thể hiện luồng dữ liệu và sự phụ thuộc giữa các thành phần, đặc biệt ở các điểm tích hợp:
...
Nhóm có thể tham khảo mô hình RBAC (Role-Based Access Control) hoặc đề xuất cách tiếp cận khác nếu có lý do phù hợp.

### 7. Thiết kế các cơ chế bảo vệ hệ thống

Với mỗi vấn đề kỹ thuật dưới đây, trình bày giải pháp nhóm lựa chọn, giải thích cách nó hoạt động và lý do phù hợp với bài toán.

Các kỹ thuật được gợi ý nhưng nhóm có thể đề xuất giải pháp thay thế nếu lập luận thuyết phục.
...
- Circuit Breaker với các trạng thái:
  - Closed
  - Open
  - Half-Open
...
- Xác định TTL phù hợp cho từng loại dữ liệu:
  - Thông tin concert ít thay đổi có thể cache lâu.
  - Số vé còn lại cần TTL ngắn hoặc invalidate chủ động khi có giao dịch thành công.
...
README rõ ràng, đủ để người chấm có thể clone repository và chạy được hệ thống mà không cần hỏi thêm.

### Dữ liệu mẫu

Seed data hoặc script tạo dữ liệu ban đầu, bao gồm các concert mẫu:
...
Dữ liệu mẫu cần có đầy đủ loại vé, giá và sơ đồ chỗ ngồi để có thể thao tác và kiểm tra ngay sau khi khởi chạy.

## Tham khảo: Template Blueprint

Template tham khảo theo cấu trúc của OpenSpec — framework spec-driven development, gồm ba lớp tài liệu:
...
```text
mã-nhóm_mssv1_mssv2_mssv3_mssv4.txt
```
...
Thư mục `src/` chứa toàn bộ mã nguồn, kèm:
