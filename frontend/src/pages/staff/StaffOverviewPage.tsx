      ...
      <section className="staff-concert-list" aria-label="Concert dành cho nhân viên">
        {loading ? (
          ...
          <article key={concert.id} className="staff-concert-row">
            ...
            <div>
              <h2>{concert.title}</h2>
              <p><MapPin size={14} />{concert.venueName}, {concert.venueAddress}</p>
            </div>
            <Link to={`/staff/check-in?concert=${concert.id}`} aria-label={`Mở check-in cho ${concert.title}`}>
          ...
          <div className="admin-empty-state">
            ...
            <p>Concert dành cho staff sẽ xuất hiện tại đây khi backend trả về trạng thái ON_SALE.</p>
          </div>
        )}
      </section>
    </>
