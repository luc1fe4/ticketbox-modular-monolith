    ...
    <>
      ...
      <section className="admin-role-note">
        <div>
          ...
          <strong>{user?.role === 'ADMIN' ? 'Toàn hệ thống' : 'Concert do bạn sở hữu'}</strong>
        </div>
        <p>
          {user?.role === 'ADMIN'
            ? 'ADMIN có thể quản lý mọi concert qua API quản trị.'
            : 'ORGANIZER chỉ có thể chỉnh sửa concert và hạng vé do chính tài khoản tạo.'}
        </p>
      </section>
    </>
  );
